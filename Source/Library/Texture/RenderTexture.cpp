...
namespace library
{
	...
	HRESULT RenderTexture::Initialize(_In_ ID3D11Device* pDevice, _In_ ID3D11DeviceContext* pImmediateContext)
	{
		...
		// Create texture 2D used as the shadow map
		...
		hr = pDevice->CreateTexture2D(&textureDesc, nullptr, &m_texture2D);
		...
		hr = pDevice->CreateSamplerState(&samplerDesc, m_samplerClamp.GetAddressOf());
