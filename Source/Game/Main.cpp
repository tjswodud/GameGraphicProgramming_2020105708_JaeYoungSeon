...
INT WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ INT nCmdShow)
{
    ...
    std::shared_ptr<SwtubeCube> swtubeCube = std::make_shared<SwtubeCube>("swtube.dds");
    if (FAILED(game->GetRenderer()->AddRenderable(L"SwtubeCube", swtubeCube)))
    {
        return 0;
    }

    if (FAILED(game->GetRenderer()->SetVertexShaderOfRenderable(L"SwtubeCube", L"MainShader")))
    {
        return 0;
    }

    if (FAILED(game->GetRenderer()->SetPixelShaderOfRenderable(L"SwtubeCube", L"MainShader")))
    {
        return 0;
    }
