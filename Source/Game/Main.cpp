...
INT WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ INT nCmdShow)
{
    ...
    XMFLOAT4 color;
    XMStoreFloat4(&color, Colors::WhiteSmoke);
    ...
    std::shared_ptr<Cube> reflectionCube = std::make_shared<Cube>(color);
    reflectionCube->Translate(XMVectorSet(0.0f, 3.0f, 6.0f, 0.0f));
    if (FAILED(mainScene->AddRenderable(L"ReflectionCube", reflectionCube)))
    {
        return 0;
    }
    if (FAILED(mainScene->SetVertexShaderOfRenderable(L"ReflectionCube", L"EnvironmentMapShader")))
    {
        return 0;
    }
    if (FAILED(mainScene->SetPixelShaderOfRenderable(L"ReflectionCube", L"EnvironmentMapShader")))
    {
        return 0;
    }
